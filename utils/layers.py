...
from tensorflow.keras.layers import PReLU, ELU, LeakyReLU, Activation, Conv2DTranspose, \
...
def BatchNorm(momentum=0.99, training=True):
    ...
    def f(x):
        return Lambda(batchnorm, output_shape=tuple([xx for xx in x.shape if xx is not None]))(x)
