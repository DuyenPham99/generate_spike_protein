...
def load_rardata(filename, one_hot=True):
    names, seqs = read_rarfasta(filename)
    if one_hot:
        seqs = to_one_hot(seqs)
    return names, seqs
...
def read_rarfasta(filepath, output_arr=False, encoding='utf-8'):
    names = []
    seqs = []
    seq = ''
    with ZipFile(filepath, 'r') as fin:
        file_content = fin.extractall()
        lines = file_content.split('\n')
        for line in lines:
            if line:
                if line[0] == '>':
